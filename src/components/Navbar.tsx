    ...
    <nav className="sticky top-0 z-50 border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="container mx-auto px-4 h-16 flex items-center justify-between">
        <Link to="/" className="flex items-center space-x-2">
          ...
          <span className="font-bold text-xl">CoinPlatform</span>
        </Link>
          ...
          <div className="container mx-auto px-4 py-4 space-y-2">
            {allLinks.map((link) => (
              <Link
                ...
                className={cn(
                  "block px-4 py-2 rounded-md text-foreground hover:text-primary hover:bg-accent transition-colors",
                  location.pathname === link.to && "text-primary bg-accent font-medium"
                )}
              >
                {link.label}
              </Link>
